...
exports.CreateTransactionFromDTO = exports.extractMosaics = exports.extractRecipient = void 0;
/*
 * Copyright 2019 NEM
 *
...
const extractMosaics = (mosaics) => {
    ...
    return mosaics.map((mosaicDTO) => {
        const id = utils_1.UnresolvedMapping.toUnresolvedMosaic(mosaicDTO.id);
        return new mosaic_1.Mosaic(id, model_1.UInt64.fromNumericString(mosaicDTO.amount));
    });
};
exports.extractMosaics = extractMosaics;
/**
...
const extractTransactionMeta = (meta, id) => {
    ...
    if (meta.aggregateHash || meta.aggregateId) {
        return new transaction_1.AggregateTransactionInfo(model_1.UInt64.fromNumericString(meta.height), meta.index, id, model_1.UInt64.fromNumericString(meta.timestamp), meta.feeMultiplier, meta.aggregateHash, meta.aggregateId);
    }
    return new transaction_1.TransactionInfo(model_1.UInt64.fromNumericString(meta.height), meta.index, id, model_1.UInt64.fromNumericString(meta.timestamp), meta.feeMultiplier, meta.hash, meta.merkleComponentHash);
};
/**
 * @internal
...
const CreateStandaloneTransactionFromDTO = (transactionDTO, transactionInfo) => {
    ...
    switch (type) {
        ...
        case transaction_1.TransactionType.MOSAIC_DEFINITION:
            return new transaction_1.MosaicDefinitionTransaction(networkType, version, deadline, maxFee, mosaic_1.MosaicNonce.createFromNumber(transactionDTO.nonce), new mosaic_1.MosaicId(transactionDTO.id), new mosaic_1.MosaicFlags(transactionDTO.flags), transactionDTO.divisibility, model_1.UInt64.fromNumericString(transactionDTO.duration), signature, signer, transactionInfo).setPayloadSize(transactionDTO.size);
        case transaction_1.TransactionType.MOSAIC_SUPPLY_CHANGE:
            return new transaction_1.MosaicSupplyChangeTransaction(networkType, version, deadline, maxFee, utils_1.UnresolvedMapping.toUnresolvedMosaic(transactionDTO.mosaicId), transactionDTO.action, model_1.UInt64.fromNumericString(transactionDTO.delta), signature, signer, transactionInfo).setPayloadSize(transactionDTO.size);
        case transaction_1.TransactionType.MOSAIC_SUPPLY_REVOCATION:
            return new model_1.MosaicSupplyRevocationTransaction(networkType, version, deadline, maxFee, (0, exports.extractRecipient)(transactionDTO.sourceAddress), new mosaic_1.Mosaic(new mosaic_1.MosaicId(transactionDTO.mosaicId), model_1.UInt64.fromNumericString(transactionDTO.amount)), signature, signer, transactionInfo).setPayloadSize(transactionDTO.size);
        ...
        case transaction_1.TransactionType.HASH_LOCK:
            return new transaction_1.LockFundsTransaction(networkType, version, deadline, maxFee, new mosaic_1.Mosaic(new mosaic_1.MosaicId(transactionDTO.mosaicId), model_1.UInt64.fromNumericString(transactionDTO.amount)), model_1.UInt64.fromNumericString(transactionDTO.duration), new transaction_1.SignedTransaction('', transactionDTO.hash, '', transaction_1.TransactionType.AGGREGATE_BONDED, networkType), signature, signer, transactionInfo).setPayloadSize(transactionDTO.size);
        case transaction_1.TransactionType.SECRET_LOCK:
            const recipientAddress = transactionDTO.recipientAddress;
            const mosaicId = utils_1.UnresolvedMapping.toUnresolvedMosaic(transactionDTO.mosaicId);
            return new transaction_1.SecretLockTransaction(networkType, version, deadline, maxFee, new mosaic_1.Mosaic(mosaicId, model_1.UInt64.fromNumericString(transactionDTO.amount)), model_1.UInt64.fromNumericString(transactionDTO.duration), transactionDTO.hashAlgorithm, transactionDTO.secret, (0, exports.extractRecipient)(recipientAddress), signature, signer, transactionInfo).setPayloadSize(transactionDTO.size);
        ...
        case transaction_1.TransactionType.MOSAIC_ALIAS:
            return new transaction_1.MosaicAliasTransaction(networkType, version, deadline, maxFee, transactionDTO.aliasAction, namespace_1.NamespaceId.createFromEncoded(transactionDTO.namespaceId), new mosaic_1.MosaicId(transactionDTO.mosaicId), signature, signer, transactionInfo).setPayloadSize(transactionDTO.size);
        ...
        case transaction_1.TransactionType.ACCOUNT_MOSAIC_RESTRICTION:
            return new transaction_1.AccountMosaicRestrictionTransaction(networkType, version, deadline, maxFee, transactionDTO.restrictionFlags, transactionDTO.restrictionAdditions
                ? transactionDTO.restrictionAdditions.map((addition) => utils_1.UnresolvedMapping.toUnresolvedMosaic(addition))
                : [], transactionDTO.restrictionDeletions
                ? transactionDTO.restrictionDeletions.map((deletion) => utils_1.UnresolvedMapping.toUnresolvedMosaic(deletion))
                : [], signature, signer, transactionInfo).setPayloadSize(transactionDTO.size);
        ...
        case transaction_1.TransactionType.MOSAIC_METADATA:
            return new transaction_1.MosaicMetadataTransaction(networkType, version, deadline, maxFee, (0, exports.extractRecipient)(transactionDTO.targetAddress), model_1.UInt64.fromHex(transactionDTO.scopedMetadataKey), utils_1.UnresolvedMapping.toUnresolvedMosaic(transactionDTO.targetMosaicId), transactionDTO.valueSizeDelta, __1.Convert.hexToUint8(transactionDTO.value), signature, signer, transactionInfo).setPayloadSize(transactionDTO.size);
