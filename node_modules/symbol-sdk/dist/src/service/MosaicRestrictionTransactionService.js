...
class MosaicRestrictionTransactionService {
    ...
    createMosaicGlobalRestrictionTransaction(deadline, networkType, mosaicId, restrictionKey, restrictionValue, restrictionType, referenceMosaicId = new MosaicId_1.MosaicId(UInt64_1.UInt64.fromUint(0).toDTO()), maxFee = new UInt64_1.UInt64([0, 0])) {
        ...
        return this.getResolvedMosaicId(mosaicId).pipe((0, operators_1.mergeMap)((resolvedMosaicId) => this.getGlobalRestrictionEntry(resolvedMosaicId, restrictionKey).pipe((0, operators_1.map)((restrictionEntry) => {
    ...
    createMosaicAddressRestrictionTransaction(deadline, networkType, mosaicId, restrictionKey, targetAddress, restrictionValue, maxFee = new UInt64_1.UInt64([0, 0])) {
        this.validateInput(restrictionValue);
        const combinedUnresolved = (0, rxjs_1.combineLatest)([this.getResolvedMosaicId(mosaicId), this.getResolvedAddress(targetAddress)]);
        return combinedUnresolved.pipe((0, operators_1.mergeMap)(([resolvedMosaicId, resolvedAddress]) => this.getGlobalRestrictionEntry(resolvedMosaicId, restrictionKey).pipe((0, operators_1.mergeMap)((restrictionEntry) => {
            if (!restrictionEntry) {
                throw new Error(`Global restriction for mosaic: ${mosaicId.toHex()} is not valid for with RestrictionKey: ${restrictionKey}`);
            }
            return this.getAddressRestrictionEntry(resolvedMosaicId, restrictionKey, resolvedAddress).pipe((0, operators_1.map)((optionalValue) => {
    ...
    getAddressRestrictionEntry(mosaicId, restrictionKey, targetAddress) {
        return this.restrictionMosaicRepository.search({ mosaicId, targetAddress, entryType: model_1.MosaicRestrictionEntryType.ADDRESS }).pipe((0, operators_1.map)((mosaicRestriction) => {
            var _a;
    ...
    getGlobalRestrictionEntry(mosaicId, restrictionKey) {
        return this.restrictionMosaicRepository.search({ mosaicId, entryType: model_1.MosaicRestrictionEntryType.GLOBAL }).pipe((0, operators_1.map)((mosaicRestrictionPage) => {
    ...
    getResolvedMosaicId(unresolvedMosaicId) {
        if (!unresolvedMosaicId.isNamespaceId()) {
            return (0, rxjs_1.of)(unresolvedMosaicId);
        }
        const namespaceId = unresolvedMosaicId;
        return this.namespaceRepository.getLinkedMosaicId(namespaceId).pipe((0, operators_1.map)((mosaicId) => {
            if (!mosaicId) {
